...
### branch
claude/m3-p3-pdf-kbc

### intent
The KBC Mastercard uitgavenstaat template on the M3-P2 foundation: fingerprint, the two-date row shape, FX continuation lines folded into their rows, the previous-balance and Afrekening lines as the balance identity, the settlement credit as a real transaction, hash dedup with the occurrence ordinal witnessed through PDF, and the account-side settlement pairing witnessed end to end across two PDF imports.

### grounding
pulse-v0.2-pdf-addendum.md:73 through :88 (template B), :91 and :100 (the section 5 amendments this phase re-witnesses through PDF; both verified already implemented at e4ea3ba, see report-code-disagreement). Verified nuances from the real statement (notes/export-format-facts.md, 2026-08-21 section): the closing figure sits on the Afrekening line with a space thousands separator, the Totaal bedrag line carries no amount, extraction word spacing is tolerance-sensitive, and the DOMICILIERING credit equals the negated previous balance.

### severity
high

### verified-root-cause
Not a defect phase. No verification-first step: every claim this phase builds on verified true against the real statement and the tree (the two false claims of this plan are owned by M3-P2). The balance gate, spec union, extraction and registry arrive from M3-P2.

### steps
- text: The KBC template per addendum section 4 and the verified structure: fingerprint on the KBC-Mastercard plus Uitgavenstaat header text, tolerant of interior label spacing; account identity from the masked card number through the profile's account binding (card files carry no IBAN, upload-statement.ts:113); booking date is the transaction date, settlement date kept in the raw description block; FX continuation lines (original amount and exchange rate) folded into the preceding row's rawLine, never rows; the DOMICILIERING credit parsed as a transaction; the previous-balance and Totaal bedrag and Afrekening lines parsed as balances, with the closing figure read from the Afrekening line including its space thousands separator.
- text: Balance identity for this template: previous balance plus the sum of all rows including the settlement credit must equal the Afrekening amount in integer cents, else FAILED "balance-mismatch" with zero rows, through the shared M3-P2 gate.
- text: Synthetic KBC fixture via the D-3 generator: invented card number, merchants and amounts reproducing FX continuation lines, the settlement credit equal to the negated previous balance, the identical duplicate row pair, at least one row whose transaction date and settlement date fall in DIFFERENT months (finding PR2-004), and reconciling balances; plus a companion Belfius-side fixture carrying the settlement debit whose amount equals this card fixture's Afrekening total, so the pairing is witnessable.
- text: End-to-end witness of the already-implemented interpretation machinery through PDF: with the card account declared, the duplicate pair both survive import and re-import (addendum section 5's named test, through the PDF path), the DOMICILIERING credit classifies INTERNAL as the mirror leg (classify-flow.ts:90), and the account-side settlement debit pairs INTERNAL to the card import (classify-flow.ts:87) with the SPEND fallback intact when no card import matches.

### files-to-touch
- src/modules/import/domain/ (kbc template file, registry entry)
- test/fixtures/ (generator additions and generated synthetic PDFs)
- test/domain/
- test/application/
...
### acceptance
- id: 3.1
  criterion: npm test exits 0 including new tests asserting, against the synthetic KBC fixture: the expected row count with FX continuation lines contributing zero rows and appearing verbatim in their row's rawLine; the DOMICILIERING credit present as a row with positive cents; the space-thousands Afrekening amount parsed to its expected integer cents; the month-straddling row's stored bookingDate equal to its TRANSACTION date, not its settlement date, so spend lands in the month it happened (pulse-v0.2-pdf-addendum.md:76, finding PR2-004); and the balance identity gate passing this fixture and failing a non-reconciling KBC variant with "balance-mismatch" and zero rows written.
- id: 3.2
  criterion: npm test exits 0 including the duplicate-pair test through the PDF path: both identical rows survive the first import, and re-importing the same fixture adds zero and reports all known, with both rows still present.
- id: 3.3
  criterion: npm test exits 0 including an application-level test importing the KBC fixture and the companion Belfius fixture into one household with declared accounts, asserting the account-side settlement debit is INTERNAL and linked to the KBC import, the DOMICILIERING credit is INTERNAL, and removing the KBC import from the scenario leaves the settlement debit SPEND against the card issuer.
- id: 3.4
  criterion: npm run test:e2e exits 0 with the import journey extended: uploading both fixture PDFs, the month view for the fixture month renders the reconciliation panel with data-state "ok".
- id: 3.5
  criterion: Privacy gate, both halves executed with a probe list read from the real PDFs in the container uploads at review time and never committed (finding PR2-008): (a) git grep across the whole worktree for the real KBC statement's card number fragments, customer reference, statement number and each merchant string from it exits 1; (b) over the files this phase adds or modifies (git diff --name-only against the phase base), the same grep extended to all five promised categories (identifiers, names, merchant strings, thousands-form amounts and dates from the real statement) exits 1. If the real PDFs are absent from the executing container, this criterion is recorded as not-witnessed in the work history and the review escalates for a container that has them, never marked green by vacuity.
- id: 3.6
  criterion: npm run typecheck && npm run lint && npm test exit 0 and npm run test:e2e exits 0 at the phase head.

### hazard-classes
- id: H3.1
  statement: Real statement content leaking into the repository through the KBC fixtures or tests.
  addressed-by: criterion 3.5
- id: H3.2
  statement: FX continuation lines parsed as transactions, double-counting foreign spend; or the settlement credit skipped as a balance line, breaking both the balance identity and the settlement pairing.
  addressed-by: criterion 3.1
- id: H3.3
  statement: The dedup key collapsing the legitimate identical pair, silently dropping a real payment while the books still close per file.
  addressed-by: criterion 3.2
- id: H3.4
  statement: The settlement debit counted as SPEND while its card rows are also imported, double-counting an entire card statement in the month view.
  addressed-by: criterion 3.3

### migrations
none, template and fixtures only.
...
### citations
- delivery/intake/pulse-v0.2-pdf-addendum.md:73
- delivery/intake/pulse-v0.2-pdf-addendum.md:83
- delivery/intake/pulse-v0.2-pdf-addendum.md:91
- src/modules/ledger/domain/classify-flow.ts:77
- src/modules/ledger/domain/constants.ts:26
- src/modules/import/application/upload-statement.ts:113

### fill-in
filled: true
root-cause: Not a defect; second layout template on the M3-P2 foundation, every structural claim verified against the real statement in-container.
fix-shape: KBC template file plus registry entry, synthetic KBC and companion Belfius fixtures, duplicate-pair and settlement-pairing witnesses.
files:
